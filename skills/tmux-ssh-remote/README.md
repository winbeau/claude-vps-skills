...
./tmux-ensure.sh "$SESSION" 'ssh -p 2222 user@host'

# 3. 注入每条后续命令（-t 超时秒，--auto-recover 超时自愈重试一次）
./tmux-inject.sh -t 120 --auto-recover "$SESSION" 'cd ~/proj && uv run pytest'
```

> 安装到 `~/.claude/skills/` 后，实际路径为 `~/.claude/skills/tmux-ssh-remote/tmux-{ensure,inject}.sh`。
...
| `tmux-ensure.sh` | 幂等创建/探活会话，落盘 ssh-cmd（`--force` 重建） |
| `tmux-inject.sh` | 向会话注入命令，带超时 + `--auto-recover` 自愈重试 |
